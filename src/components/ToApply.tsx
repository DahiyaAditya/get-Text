...
import { formatDate, searchLinkedInURL } from '../utils';
...
export default function ToApply({ list, onAdd, onDelete, onUpdate, onGenerate, activeFilter, onFilterChange }: ToApplyProps) {
  ...
  const constructLinkedInUrl = (companyName: string): string => {
    ...
    return `https://www.linkedin.com/company/${slugifiedName}`;
  };
  const handleParseJob = async () => {
    if (!jobUrl || !jobSummary) return;
    setIsParsing(true);
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Parse this job opening content and extract all relevant details: ${jobSummary}. 
        Please provide the details in a structured format. pls don't add anything which is not present in content`,
        config: {
          responseMimeType: "application/json",
          responseSchema: {
            type: Type.OBJECT,
            properties: {
              company: { type: Type.STRING, description: "Base name of the company without legal suffixes (e.g., 'Siemens' not 'Siemens Private Technology Ltd')" },
              ...
              linkedinUrl: { type: Type.STRING, description: `Find the LinkedIn company profile URL for companyName". Please search your knowledge base and provide the direct LinkedIn company page URL. Only return the LinkedIn company URL, nothing else. If you cannot find it, respond with "NOT_FOUND".`, },
            },
      ...
      if (!result.linkedinUrl || result.linkedinUrl === 'NOT_FOUND') {
        if (result.company) {
          result.linkedinUrl = constructLinkedInUrl(result.company);
        }
      }
      setParsedJob(result);
      ...
      <div className="bg-white p-5 md:p-8 rounded-[32px] shadow-sm border border-black/5">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 px-5 py-2.5 rounded-2xl text-sm font-bold transition-all ${isAdding ? 'bg-gray-100 text-gray-600' : 'bg-black text-white shadow-md hover:bg-gray-800'
                }`}
            >
        ...
        <div className="flex items-center gap-2 mb-8 p-1 bg-gray-50 rounded-2xl w-fit border border-gray-100">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${activeFilter === 'to-apply'
              ? 'bg-white text-black shadow-sm border border-gray-100'
              : 'text-gray-400 hover:text-gray-600'
              }`}
          >
            To Apply
            <span className={`px-2 py-0.5 rounded-full text-[10px] ${activeFilter === 'to-apply' ? 'bg-black text-white' : 'bg-gray-200 text-gray-500'
              }`}>
              {toApplyList.length}
            </span>
          </button>
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${activeFilter === 'applied'
              ? 'bg-white text-black shadow-sm border border-gray-100'
              : 'text-gray-400 hover:text-gray-600'
              }`}
          >
            Applied
            <span className={`px-2 py-0.5 rounded-full text-[10px] ${activeFilter === 'applied' ? 'bg-black text-white' : 'bg-gray-200 text-gray-500'
              }`}>
              {appliedList.length}
            </span>
          </button>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e: { target: { value: any; }; }) => setNewToApply({ ...newToApply, company: e.target.value })}
                    />
                  </div>
                </div>
                <div className="space-y-1.5">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredList.length === 0 ? (
                ...
                filteredList.map((item) => (
                  <tr
                    key={item.id}
                    className={`transition-colors group ${item.applied ? 'bg-emerald-50/50 hover:bg-emerald-100/50' : 'hover:bg-gray-50/50'
                      }`}
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-xs font-bold ${new Date(item.lastDate) < new Date() ? 'text-red-400' : 'text-orange-500'
                        }`}>
        ...
        <div className="md:hidden space-y-4">
          {filteredList.length === 0 ? (
            ...
            filteredList.map((item) => (
              <div
                key={item.id}
                className={`p-5 rounded-2xl border transition-colors space-y-4 ${item.applied ? 'bg-emerald-50 border-emerald-100' : 'bg-gray-50 border-gray-100'
                  }`}
              >
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <hr></hr>
                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-400 uppercase px-1">Job Summary</label>
                  <div className="relative">
                    <textarea
                      placeholder="Copy Summary"
                      className="w-full pl-4 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 resize-none"
                      rows={6}
                      value={jobSummary}
                ...
                <button
                  ...
                  disabled={isParsing || !jobUrl || !jobSummary}
