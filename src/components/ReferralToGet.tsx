...
interface ReferralToGetProps {
  ...
  onDelete: (item: ReferralToGetItem) => void;
}

export default function ReferralToGet({ list, onAddManual, onMoveToGot, onDelete }: ReferralToGetProps) {
          ...
          return (
            <div key={item.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-5 md:p-6 bg-gray-50 rounded-2xl border border-gray-100 gap-4">
              ...
              <button
                ...
                className="w-full sm:w-auto bg-red-500 text-white px-6 py-3 md:py-2.5 rounded-xl text-sm font-bold hover:bg-red-600 transition-all shadow-sm shrink-0"
              >
                Delete
              </button>
            </div>
