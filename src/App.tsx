...
function AppContent() {
  ...
  const deleteToGet = async (item: ReferralToGetItem) => {
    try {
      await deleteDoc(doc(db, 'referralToGet', item.id));
    } catch (error) {
      handleFirestoreError(error, OperationType.DELETE, `referralToGet/${item.id}`);
    }
  };
  const deleteGot = async (id: string) => {
    try {
      ...
      handleFirestoreError(error, OperationType.DELETE, `referralToGot/${id}`);
    }
  };
          ...
          <nav className="hidden lg:flex items-center bg-gray-100/50 p-1 rounded-2xl border border-gray-200/50">
            {[
              ...
              <button
                ...
                className={`relative flex items-center gap-2 px-3 xl:px-4 py-2 rounded-xl text-xs font-bold transition-all duration-300 ${activeTab === tab.id
                  ? 'text-black'
        ...
        <div className="flex overflow-x-auto no-scrollbar items-center gap-8 px-8 py-3">
          {[
            ...
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as TabType)}
              className={`flex flex-col items-center gap-1 transition-all shrink-0 ${activeTab === tab.id ? 'text-black' : 'text-gray-400'
                }`}
            >
